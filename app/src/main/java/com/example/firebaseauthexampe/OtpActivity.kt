        ...
        auth.signInWithCredential(credential)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    ...
                    Log.d("Signin Credential error", "signInWithCredential:success")
                    progressBar.visibility = View.INVISIBLE
                    Toast.makeText(this@OtpActivity, "", Toast.LENGTH_SHORT).show()
                    sendToMain()
                } else {
                    // Sign in failed, display a message and update the UI
                    Log.w("Signin Credential error", "signInWithCredential:failure", task.exception)
                    if (task.exception is FirebaseAuthInvalidCredentialsException) {
                        // The verification code entered was invalid
                        Toast.makeText(this@OtpActivity, "task.exception.message", Toast.LENGTH_SHORT).show()
                    }
                    // Update UI
                }
            }
